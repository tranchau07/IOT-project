  ...
  return (
    <div className="space-y-6 py-6 select-none">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-extrabold tracking-tight text-slate-900 flex items-center gap-2">
            <LayoutGrid className="h-5.5 w-5.5 text-blue-600" />
            System Cockpit Overview
          </h2>
          <p className="text-xs text-slate-500 font-semibold mt-1">Realtime monitoring and controls of smart rooms</p>
        </div>

        <div className="flex gap-2">
          <button
            onClick={fetchClassrooms}
            className="p-2.5 rounded-xl border border-slate-200 bg-white text-slate-650 hover:bg-slate-50 hover:text-slate-900 transition-all cursor-pointer shadow-sm"
            title="Refresh Grid"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden shadow-sm">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Smart Rooms</span>
            <h3 className="text-xl font-extrabold text-slate-900 mt-0.5">{totalClassrooms}</h3>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden shadow-sm">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Online Nodes</span>
            <h3 className="text-xl font-extrabold text-emerald-600 mt-0.5">{onlineDevices}</h3>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden shadow-sm">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Active ACs</span>
            <h3 className="text-xl font-extrabold text-slate-900 mt-0.5">{activeAcs}</h3>
          </div>
        </div>

        <div className={`bg-white border rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden shadow-sm transition-all duration-200 ${
          systemFaults > 0 ? 'border-rose-250 bg-rose-50/30' : 'border-slate-200/80'
        }`}>
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">Active Alarms</span>
            <h3 className={`text-xl font-extrabold mt-0.5 ${systemFaults > 0 ? 'text-rose-600 animate-pulse' : 'text-slate-900'}`}>
              {systemFaults}
            </h3>
          </div>
        </div>
      ...
      {isLoading && classrooms.length === 0 ? (
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-12 text-center space-y-4 shadow-sm">
          <School className="h-12 w-12 text-slate-350 mx-auto" />
          <h3 className="text-sm font-bold text-slate-800">No Classroom Nodes Provisioned</h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden relative border border-slate-100 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-4.5 border-b border-slate-100">
              <h3 className="font-extrabold text-slate-950">Provision New Room Node</h3>
              <button
            ...
            <form onSubmit={handleCreateClassroom} className="p-6 space-y-4.5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Classroom Name *</label>
                  <input
                    type="text"
                    required
                    value={name}
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-white border border-slate-200 focus:border-blue-500 text-slate-800 text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/10"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Building Name *</label>
                  <input
                    type="text"
                    required
                    value={building}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Device Hardware ID *</label>
                  <input
                    type="text"
                    required
                    value={deviceId}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Gateway Category</label>
                  <select
                    value={deviceType}
                    onChange={(e) => setDeviceType(e.target.value)}
                    className="w-full px-3.5 py-2 rounded-xl bg-white border border-slate-200 focus:border-blue-500 text-slate-650 text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/10 cursor-pointer"
                  >
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Light Relays Count</label>
                  <input
                    type="number"
                    min={1}
                    max={8}
                    ...
                    className="w-full px-3.5 py-2 rounded-xl bg-white border border-slate-200 text-slate-800 text-sm font-semibold focus:outline-none"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Fan Relays Count</label>
                  <input
                    type="number"
                    min={1}
                    max={8}
